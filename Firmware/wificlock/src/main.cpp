...
//---------------修改此处""内的信息--------------------
const char ssid[] = "LXC";      //WIFI名称 修改这2个就可以了
const char pass[] = "12223641";    //WIFI密码
//----------------------------------------------------
...
String cityCode = "101210101";  //天气城市代码 长沙:101250101株洲:101250301衡阳:101250401
  ...
  if(Serial.available()>0)
  {
    ...
    if(SMOD=="0x01")//设置1亮度设置
    {
      ...
      if(LCDBL>=0 && LCDBL<=100)
      {
        ...
        SMOD = "";
        Serial.printf("亮度调整为：");
        analogWrite(LCD_BL_PIN, 250 - (LCD_BL_PWM*2.5));
...
void setup()
{
  ...
  Udp.begin(localPort);
  Serial.println("等待同步...");
  setSyncProvider(getNtpTime);  //获取当前时间
  setSyncInterval(300);         //设置每300秒刷新一次时间
